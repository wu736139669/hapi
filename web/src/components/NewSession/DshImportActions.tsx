...
export function DshImportActions(props: {
    selectedSession: DshLocalSessionSummary | null
    isLoading: boolean
    isDisabled: boolean
    error: string | null
        ...
        <div className="flex flex-col gap-2 px-3 py-3">
            <div className="flex items-center justify-between gap-2">
                <div className="min-w-0">
                    <div className="text-xs font-medium text-[var(--app-hint)]">{t('dshImport.inline.title')}</div>
                    <div className="truncate text-[11px] text-[var(--app-hint)]">
                        {props.selectedSession ? props.selectedSession.title : t('dshImport.inline.description')}
                    </div>
                </div>
                <div className="flex shrink-0 items-center gap-2">
                    {props.selectedSession ? (
                        <button type="button" className="text-xs text-[var(--app-link)]" onClick={props.onClear} disabled={props.isDisabled}>
                            {t('dshImport.inline.clear')}
                        </button>
                    ) : null}
                    <button
                        type="button"
                    ...
                    >
                        {props.isLoading ? t('dshImport.loading') : t('dshImport.inline.choose')}
                    </button>
                </div>
            </div>
