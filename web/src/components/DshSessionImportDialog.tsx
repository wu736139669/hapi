...
import type { DshLocalSessionSummary } from '@/types/api'
...
export function DshSessionImportDialog(props: {
    isOpen: boolean
    onClose: () => void
        ...
        <Dialog open={props.isOpen} onOpenChange={(open) => !open && props.onClose()}>
            <DialogContent className="max-w-xl">
                <DialogHeader className="text-left">
                    <DialogTitle>{t('dshImport.dialog.title')}</DialogTitle>
                    <DialogDescription>{t('dshImport.dialog.description')}</DialogDescription>
                </DialogHeader>
                <div className="mt-3 space-y-3">
                    ...
                    {props.sessions.length > 0 ? (
                        <div className="grid gap-2 sm:grid-cols-2">
                            <SelectControl value={directory} disabled={props.isPending} onChange={(event) => setDirectory(event.target.value)}>
                                <option value={ALL_DIRECTORIES}>{t('dshImport.allDirectories')}</option>
                            ...
                            <input
                                ...
                                placeholder={t('dshImport.search')}
                    ...
                    <div className="max-h-[50vh] overflow-y-auto rounded-lg border border-[var(--app-border)]">
                        {props.isLoading ? (
                            <div className="px-4 py-8 text-center text-sm text-[var(--app-hint)]">{t('dshImport.loading')}</div>
                        ) : filtered.length === 0 ? (
                            <div className="px-4 py-8 text-center text-sm text-[var(--app-hint)]">{t('dshImport.empty')}</div>
                        ) : (
                                        ...
                                        <label key={session.id} className="flex cursor-pointer items-start gap-3 px-3 py-2 hover:bg-[var(--app-subtle-bg)]">
                                            ...
                                            <span className="min-w-0 flex-1">
                                                <span className="flex items-center gap-2 text-sm font-medium text-[var(--app-fg)]">
                                                    ...
                                                    {session.hapiSessionId ? <span className="rounded bg-[var(--app-subtle-bg)] px-1.5 py-0.5 text-[10px] text-[var(--app-hint)]">{t('dshImport.imported')}</span> : null}
                                                    {session.importState === 'diverged' || session.importState === 'failed' ? <span className="text-[10px] text-red-600">{t('dshImport.needsAttention')}</span> : null}
                                                </span>
                                                ...
                                                <span className="mt-0.5 block text-[10px] text-[var(--app-hint)]">
                                                    {new Date(session.modifiedAt).toLocaleString()} · {session.messageCount} {t('dshImport.messages')}
                                                </span>
                                            </span>
                                        </label>
                                    )
                    ...
                    <div className="flex justify-end gap-2">
                        ...
                        <Button type="button" onClick={() => void props.onConfirm(selectedIds)} disabled={selectedIds.length === 0 || props.isPending || props.isLoading}>
                            {props.isPending ? t('dshImport.importing') : t('dshImport.confirm')}
                        </Button>
                    </div>
                </div>
            </DialogContent>
