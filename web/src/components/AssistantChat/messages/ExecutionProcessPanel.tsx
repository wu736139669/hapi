...
export function getExecutionProcessGroupPaths(
    ...
    // Presentation preference only: when disabled, render every part through
    // the normal assistant message path instead of wrapping activity in the
    // fixed-height execution panel.
    if (options.enabled === false) return parts.map(() => [])
